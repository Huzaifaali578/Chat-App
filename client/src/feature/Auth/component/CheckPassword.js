...
import { selectCheckEmailUser, selectLoggedInUser, signInApiAsync } from '../authSlice';
...
function CheckPassword() {
  ...
  const handleLogin = () => {
    if (!isPasswordEmpty) {
      dispatch(signInApiAsync({ password: passwordRef.current, userId }));
    }
  };
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className={`px-3 py-2 ml-2 bg-slate-100 focus:outline-primary rounded-md}`}
              onChange={handlePasswordChange}
              ref={passwordRef}
              required
            />
          </div>
          {isPasswordEmpty && <p className="text-red-500 text-sm">Password is required.</p>}
        </div>
